...
describe('Unicode and Encoding Tests', () => {
  ...
  it('should encode/decode round-trip for Unicode', () => {
...
describe('Flat Keys Tests (No Hierarchy)', () => {
  const parser = new PropertiesParser();

  it('should handle random keys without hierarchy', () => {
  ...
  it('should handle single letter keys', () => {
    const content = 'a=1\nb=2\nc=3';
    const result = parser.parse(content);
    assert.strictEqual(result.length, 3);
  ...
  it('should handle long flat keys', () => {
...
describe('Configuration File Tests', () => {
  ...
  it('should parse configuration format (URL, paths)', () => {
...
describe('Add Key Tests', () => {
  const parser = new PropertiesParser();
  const serializer = new PropertiesSerializer();

  it('should maintain alphabetical order when adding keys', () => {
    const entries = [
  ...
  it('should group by prefix when adding keys', () => {
    const entries = [
